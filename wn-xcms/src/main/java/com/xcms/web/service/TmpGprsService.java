...
//			}
	 ...
	 * @return
	 */
	public List<TmpGprs> search(String bdcode,String phonenum,String loginName,int start,int limit) throws Exception{
		Cnd condition = null;
		if(null != phonenum && !"".equals(phonenum)){
			condition = Cnd.where("d_phone_number","=",phonenum).and("d_uid","=",loginName.replace("weinan2", "weinan1")).and("", "=",bdcode);
			start = start/limit+1;
			if(start == 0){
				start = 1;
			}

...
public class PCPOPHtmlParserBK {
	
	static Log logger = LogFactory.getLog(PCPOPHtmlParserBK.class);
    ...
    private static ScheduledExecutorService exec =
        Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
            public Thread newThread(Runnable r) {
                return new Thread(r, "PCPOP-thread");
            }
        });
	/**
	 * 获取分类链接
	...
	{
		logger.info(" >> 开始解析站点["+bean.getName()+"|"+bean.getUrl()+"]");
		try{
			Long start1 = System.currentTimeMillis();
			String content = ViewSourceFrame(bean.getUrl());
			if(null != content && !"".equalsIgnoreCase(content)){
	...
	static void docByHTML(String content, String pre) throws Exception {
		...
		if (list != null && list.size() > 0) {
			...
			if (linkList != null && linkList.size() > 0) {
				for (int i = 0; i < linkList.size(); i++) {
					LinkTag link = (LinkTag) linkList.elementAt(i);
//					logger.debug("title:"+link.getLinkText()+"\t"+link.getLink());
					LinkBean bean = null;
					if (link.getLink().toLowerCase().startsWith(pre) && !link.getLinkText().equalsIgnoreCase("详细内容")){
...
//							logger.debug(">> 已存在 ["+link.getLink()+"] 地址");
//						}
					}
				}
			}
...
//			logger.debug("author:"+tmp);
	...
	public static void main(String args[]) {
		start();
		try {
			String author = author("http://www.pcpop.com/doc/0/720/720856.shtml");
...
//			initHTML(weblist);
		        exec.schedule(new Runnable() {
		            public void run() {
		            	for(final WebsiteBean bean:weblist)
		            	{
			                initHTML(bean);
			                try {

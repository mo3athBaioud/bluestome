...
public class EsbSignalCodecUtils {
	...
	/*	from ESB服务端内部模块交互协议.txt
	or http://172.16.3.102/redmine/boards/9/topics/10
	"
	#define ESB_PROTOCOL_CODE_MODULE2MODULE 0x1       ESB上的模块之间的通信消息 
	#define ESB_PROTOCOL_CODE_ESB2ACCESS    0x2       ESB上模块向终端发消息， 由接入进行转发 
	#define ESB_PROTOCOL_CODE_ACCESS2ESB    0x3                  接入转发来自终端的消息到ESB上的模块。
	"
	*/
	
	/* protocolCode 定义 */
	public final static short ESB_PROTOCOL_CODE_MODULE2MODULE = 0x1;       /* ESB上的模块之间的通信消息 */
	public final static short ESB_PROTOCOL_CODE_ESB2ACCESS = 0x2;       	/* ESB上模块向终端发消息， 由接入进行转发 */
	public final static short ESB_PROTOCOL_CODE_ACCESS2ESB = 0x3;       	/* 接入转发来自终端的消息到ESB上的模块。 */
...
//    		return	null;
    ...
    private static EsbHeaderable decodeModule2Module(
        ...
        //	支持在MinaEsbDecoder中尽可能早的进行hdr相关赋值
...
//        signal.setSrcESBAddr(hdr.getSrcESBAddr());
	...
	private static byte[] encodeModule2Module( 
		...
		fixedHdr.setSrcESBAddr( ( 0 == signal.getSrcESBAddr() ) ? (short)myESBAddr : signal.getSrcESBAddr() );

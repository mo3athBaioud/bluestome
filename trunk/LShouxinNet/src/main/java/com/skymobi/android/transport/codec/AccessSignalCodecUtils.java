...
import com.skymobi.android.util.Zip;
...
public class AccessSignalCodecUtils {
    ...
    private static byte[] encodeTerminal2Access(
        ...
        //压缩的正文
/*        byte[] zipBody = null;
        //如果正文内容，不包含序号和消息码的长度大于100，则进行压缩
        if(bytesBody.length > 100){
            logger.debug("\t>>>>>> body length more than 100 bits");
            int blen = bytesBody.length;
            zipBody = Zip.zip(bytesBody);
            //获取压缩后的字节长度，如果之间的压缩的数据比原来的数据少，则将该压缩数据替换掉原理的数据
            //同时修改flags的值为带压缩和序号的flag,一般为:
            // 0x0720： 加密+压缩+带序号
            // 0x0320：压缩+带序号
            if(zipBody.length < blen){
                logger.debug("\t原始的数据大小为:"+blen);
                logger.debug("\t压缩后的数据大小为:"+zipBody.length);
                bytesBody = zipBody;
                fixedHdr.setFlags((short)0x0320);
                bodySize = bytesBody.length;
                logger.debug("\t设置压缩后的正文数据长度为:"+bodySize);
            }
        }
*/        

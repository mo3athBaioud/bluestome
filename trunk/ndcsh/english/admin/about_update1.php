...
$udate = date("YmdHis");
$rs=$conn->Execute("select count(*) as mcount from wdata_e where mtype = 'g' and stype = 'a'");
if (intval($rs->fields["mcount"]) > 0) {
	$sql="update wdata_e set f1='$f1',f2='$f2',f3='$f3',f4='$f4',f5='$f5',mtype='g',stype='a' where hotid='".$k1."'";
} else {
$sql="insert into wdata_e (hotid,f1,f2,f3,f4,f5,mtype,stype) values".

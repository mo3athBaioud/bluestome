<? 
	...
	$pmrs = $conn->Execute("select * from wdata_pic_e where hotid='".$hotid."' order by ordernum LIMIT ".$start." ,6");
...
<?	
	$dmrs = $conn->Execute("select * from wdata_e where mtype = 'b' and stype = 'b' order by ordernum");	
	while ($dmrs && !$dmrs->EOF) { 	?>				
...
?>	
					  ...
					  <td valign="top" height="220">
                        <table width="456" border="0" cellspacing="4" cellpadding="0">
                          <tr> 
                            <td valign="middle" align="center">
							
							<div style="float:left;topmargin:30px;">
<?	while ($pmrs && !$pmrs->EOF) { 	?>											
						<span style="float:left;">
						<!-- bgcolor="FFD700"  -->
						<table width="97" height="59" border="0" cellpadding="0" bgcolor="FFD700" cellspacing="1" leftmargin="1px">
                                <tr> 
                                  <td width="95">
										<div style="cursor:pointer" onclick="javascript:display_pic('club','<? echo $pmrs->fields["picture"]; ?>');">
										<img src="upload/<? echo $pmrs->fields["picture"]; ?>" border="0" width="147" height="107">
										</div>
								  </td>
                                </tr>
                              </table>

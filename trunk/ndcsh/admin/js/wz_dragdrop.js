...
//	alert('defparent:'+this.defparent);	
	if(!this.div) return;
...
//	alert('dd.id:'+dd.name+',setz:'+d_x);
	...
	var str = this.name;
	if (str.substr(0,5) == "child") {
		//alert('child,oriz:'+this.z);
		myz= Math.max(dd.z, this.z = this.div? ((this.css || this.div).zIndex = d_x) : 0);
		if (myz >= 89999) {
			myz = 50001;
		}
		dd.z = myz; 
		//alert('child,newz:'+dd.z);		
	} else {
		if (this.css.zIndex >= 90000) {
			//alert(this.name+':'+this.css.zIndex);
			//myz= Math.max(dd.z, this.z = this.div? ((this.css || this.div).zIndex = d_x) : 0);
			//dd.z = this.css.zIndex; 		
		} else {
			myz= Math.max(dd.z, this.z = this.div? ((this.css || this.div).zIndex = d_x) : 0);
			dd.z = myz; 					
		}
...
{
//	alert('dd.id:'+this.name+',max:'+dd.z);
//	if (dd.z >= 90000) {
//		alert('none');
		//this.setZ(99999, 1);
//	} else {
...
function PICK(d_ev)
{
	...
	if(dd.obj)
	{
		...
		else
		{
			...
			//Allen Mark This part 2008.03.21
			//dd.obj.maximizeZ();
...
function my_ResizeFunc()
{
	...
	//alert('dd.elements.' + dd.obj.name + '.w  = ' + dd.obj.w + '	 dd.elements.' + dd.obj.name + '.h = ' + dd.obj.h);
	scale_flag=1;
}
...
function my_DropFunc()
{
	...
	if (str.substr(0,5) == "child") {
		...
		var content = document.getElementById('page_content');
		  ...
		  objx= dd.obj.x - contentLeft;
		  objy= dd.obj.y - contentTop;
		  
		  bookid=str.substr(5,20);
		  currpage=str.substr(25,5);
		  objid=str.substr(30,5);	
//		  alert(dd.obj.z);
	//	  alert('objy:'+objy);
		//alert('contentTop:'+contentTop+',contentLeft:'+contentLeft+'dd.elements.' + dd.obj.name + '.x  = ' + objx + '	 dd.elements.' + dd.obj.name + '.y = ' + objy);	
		if ((dd.obj.x > 0) && (dd.obj.y > 0)){
			if (scale_flag==1){
				
				alert("scals");
			}			
			doFuncReturnXmlv2(shome+'brwizard/setobjpos.php','bookid='+bookid+'&currpage='+currpage+'&objid='+objid+'&startx='+objx+'&myy='+objy);
		}
		dd.obj=dd.elements.property_area;
	}
	scale_flag=0;	
}

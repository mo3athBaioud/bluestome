...
$isupdate = false;
if($action == "delete"){
	$sql = "delete from tbl_ads where d_id=".$id;
	$sg = "删除";
	if ($conn->Execute($sql)) {
...
if ($_FILES['background']['tmp_name'] != "") {						
	...
	$rs1 = $conn->Execute("select count(*) as mcount from tbl_ads where d_id=".$id);

	if (intval($rs1->fields["mcount"]) > 0) {

	$sql="update tbl_ads set d_ads_title = '$title',".
			"d_ordernum='$ordernum',".
			"d_status='$status',".
	...
	$rs2 = $conn->Execute("select count(*) as mcount from tbl_ads where d_status = 1");
	$ordernum = $rs2->fields["mcount"];
	#首先将原来的记录置为不可用
	$sql1 = "delete from tbl_ads";
	$sql="insert into tbl_ads (d_ads_title,d_ads_file_name,d_ordernum,d_status,d_createtime) values".

<? 
	...
	$mrs = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'a'");
	$mydata = $LANG."data";
	$plmrs = $conn->Execute("select count(*) as mcount from wdata where mtype = 'b' and stype = 'b' ");
                    ...
                    <tr> 
                      <td width="267" height="407" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <table width="253" height="159" border="0" cellpadding="0" cellspacing="2">
                          <tr> 
							...
							<?	
								$dmrs = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid");	
								while ($dmrs && !$dmrs->EOF) { 	?>				
					  ...
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <table width="100%" border="0" cellspacing="7" cellpadding="0" >
                          <tr>
							<?
							$rs1 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".$start.",".($start+1));
							if($rs1 && !$rs1->EOF){
							?>
							...
							<?}?>
							<?
							$rs2 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".($start+1).",".($start+2));
							if($rs2 && !$rs2->EOF){
							?>
                        ...
                        <table width="100%" border="0" cellspacing="7" cellpadding="0" >
                          <tr> 
							<?
							$rs3 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".($start+2).",".($start+3));
							if($rs3 && !$rs3->EOF){
							?>
                            <td width="55%" align="right" >
							...
							<?}?>
							<?
							$rs4 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".($start+3).",".($start+4));
							if($rs4 && !$rs4->EOF){
							?>
                            <td width="45%" align="left" >
                        ...
                        <table width="100%" border="0" cellspacing="7" cellpadding="0" >
                          <tr> 
							<?
							$rs5 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".($start+4).",".($start+5));
							if($rs5 && !$rs5->EOF){
							?>
                            <td width="55%" align="right" >
							...
							$rs6 = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum,hotid LIMIT ".($start+5).",".($start+6));
							if($rs6 && !$rs6->EOF){
							?>
                            <td width="45%" align="left" >

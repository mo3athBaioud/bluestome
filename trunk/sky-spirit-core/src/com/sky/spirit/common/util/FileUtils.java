/**
 * �ļ�com.sky.spirit.common.util.FileUtils.java ������2008 2008-9-4 ����09:38:57
 * ��Ȩ������ ˹������ SkySpirit��Ŀ
 * ������: ȫ��Ӫ
...
import com.sky.spirit.basic.exception.SkyRuntimeException;

/**
 * 
 * ע��
 * @author <a href="jonny_quan@hotmail.com">jonny</a>
 * @date 2008 2008-9-6 ����11:17:30
 * @version 1.0.0<br>
 * ���¼�¼��ע �����ˣ�����ʱ�䣬�������ݣ����汾��
 *
 */
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static File openDirectory(String directory) throws IOException {
		...
		if (!dir.isDirectory() || !dir.canRead()) {
			throw new IOException("Path: " + directory
	 ...
	 */
	public static File stringToFile(String filename, String data)
			throws IOException {
		return stringToFile(filename, data, false);
	}

	public static synchronized File stringToFile(String filename, String data,
			boolean append) throws IOException {
		return stringToFile(filename, data, append, false);
	}

	public static synchronized File stringToFile(String filename, String data,
			boolean append, boolean newLine) throws IOException {
	...
	public static String getResourcePath(String resourceName,
			Class callingClass, String encoding) throws IOException {
	 ...
	 */
	public static String normalizeFilePath(URL url, String encoding)
			throws UnsupportedEncodingException {
	 ...
	 */
	public static boolean deleteTree(File dir,
			final String[] topLevelDirsToIgnore) {
				...
				OUTER: if (files[i].isDirectory()) {
					if (topLevelDirsToIgnore != null) {
						for (int j = 0; j < topLevelDirsToIgnore.length; j++) {
							...
							if (ignored.equals(FilenameUtils
									.getBaseName(files[i].getName()))) {
								break OUTER;
							}
						}
					}
	 ...
	 * 4117557</a>. More in-context information at <a
	 * href="http://mule.mulesource.org/jira/browse/MULE-1112">MULE-1112</a>
	 * <p/> Factory methods correspond to constructors of the
	...
	public static void extractResources(String resourceName,
			Class callingClass, File outputDir, boolean keepParentDirectory)
			throws IOException {
		URL url = callingClass.getClassLoader().getResource(resourceName);
		URLConnection connection = url.openConnection();
		if (connection instanceof JarURLConnection) {
			extractJarResources((JarURLConnection) connection, outputDir,
					keepParentDirectory);
		} else {
			// extractFileResources(normalizeFilePath(url,
			//SkyServer.getMuleContext().getConfiguration().getDefaultEncoding()
			// ),
	 ...
	 *             if any error
	 */
	private static void extractFileResources(String path, File outputDir,
			String resourceName, boolean keepParentDirectory)
			throws IOException {
	 ...
	 *             if any error
	 */
	private static void extractJarResources(JarURLConnection connection,
			File outputDir, boolean keepParentDirectory) throws IOException {
	...
	public static boolean renameFileHard(String srcFilePath, String destFilePath) {
		if (StringUtils.isNotBlank(srcFilePath)
				&& StringUtils.isNotBlank(destFilePath)) {
	...
	public static boolean renameFileHard(File srcFile, File destFile) {
		...
		if (srcFile != null && destFile != null) {
			logger.debug("Moving file " + srcFile.getAbsolutePath() + " to "
			...
			if (!destFile.exists()) {
				try {
					...
					logger.debug("Error renaming file from "
							+ srcFile.getAbsolutePath() + " to "
							+ destFile.getAbsolutePath());
				}
			} else {
				logger.debug("Error renaming file " + srcFile.getAbsolutePath()
	...
	public static boolean renameFile(File srcFile, File destFile) {
		...
		if (srcFile != null && destFile != null) {
				...
				try {
					if (srcFile.isFile()) {
						...
						if (!isRenamed && srcFile.exists()) {
							logger
									.debug("Trying hard copy, assuming partition crossing ...");
					...
					logger.debug("Error moving file from "
							+ srcFile.getAbsolutePath() + " to "
			...
			logger
					.debug("Error renaming file. Source or destination file is null.");
		}
	 ...
	 * via NIO
	 */
	public static boolean moveFile1(File sourceFile, File destinationFile) {

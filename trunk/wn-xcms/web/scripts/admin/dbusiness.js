...
//				value:'1',
	...
	app.btn_reset_code = new Ext.Button({
		text : '重置号码营销状态',
		disabled:false,
		iconCls : 'icon-accept',
		handler : function(){
			...
			if(records.length == 0 ){
				Ext.Msg.show({
					title : '提示',
					msg:'已查询出手机号码记录,请选择需要重置营销状态和结果的手机号码',
					buttons : Ext.Msg.OK,
					icon : Ext.Msg.ERROR
				});
			}else{
				...
				var mSuccess = app.promptss.getValue();
					...
					Ext.Msg.confirm('提示', '你确定重置所选记录的营销状态和结果?', function(btn) {
						if (btn == 'yes') {
							Ext.Ajax.request({
								url : project+'/bussinesssimplify/admin/reset.cgi',
								params : {
									id : records[0].get('id'),
	...
	app.ds_data = new Ext.data.Store({
		url : project + '/bussinesssimplify/admin/list.cgi',
		baseParams:{},
	...
	**/
	...
	app.grid = new Ext.grid.GridPanel({
        ...
        viewConfig: {
 		...
 		plugins: expander,
		sm:app.sm,
		tbar : ['请输入手机号码：',app.text_phone_number,'-',app.btn_search_code,'-',app.btn_reset_code]
//		bbar : app.ptb
	});

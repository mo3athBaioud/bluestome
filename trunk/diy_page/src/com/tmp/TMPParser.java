...
public class TMPParser {
	...
	static String URL_ = "http://www.11xxoo.info/";
	
	static String URL = "http://www.11xxoo.info";
	...
	static void catalog(String url) throws Exception { // WebsiteBean bean
		...
		NodeList list = parser.extractAllNodesThatMatch(fileter)
				.extractAllNodesThatMatch(
						new HasAttributeFilter("class", "center dhborder clear menu"));
		if (null != list && list.size() > 0) {
			...
			if (linkList != null && linkList.size() > 0) {
				for (int i = 0; i < linkList.size(); i++) {
					...
					if(!link.getLink().startsWith("http://") && !link.getLink().startsWith("/xiazaiqu")){
	...
	static ResultBean hasPaging2(String url) throws Exception {
		...
		parser.setURL(url);
		parser.setEncoding("GB2312");
		String page = "list141{1}.html";
		// 获取指定ID的TableTag内容
		NodeFilter filter = new NodeClassFilter(CompositeTag.class);
		NodeList list = parser.extractAllNodesThatMatch(filter)
				.extractAllNodesThatMatch(
						new HasAttributeFilter("class", "pageinfo"));
		if (list != null && list.size() > 0) {
//			System.out.println(" >> hasPage:"+list.toHtml());
			String tmp = list.toHtml();
			int start = tmp.indexOf("<strong>");
			int end = tmp.indexOf("</strong>");
			int pagesize = 1;
			try{
				pagesize = Integer.valueOf(tmp.substring(start+"<strong>".length(),end));
			}catch(NumberFormatException e){
				e.printStackTrace();
			}
//			System.out.println(" >> out:"+pagesize);
			for(int i=1;i<pagesize+1;i++){
				System.out.println(url+""+page.replace("{1}", String.valueOf(i)));
			}
//			Parser p2 = new Parser();
//			p2.setInputHTML(list.toHtml());
...
//							System.out.println("小图地址:"+imgBean.getImgUrl());
	...
	public static void main(String[] args) {
		...
		try {
//			catalog(URL);
			hasPaging2("http://www.11xxoo.info/tupianqu/SSE/");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

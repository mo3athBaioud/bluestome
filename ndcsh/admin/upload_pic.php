...
#		exit;		
	...
	$csql = "select count(*) as mcount from wdata_pic where hotid = '".$hotid."' and udate = '".$uudate."'";
	#and ordernum ='".$ordernum."'
	$crs = $conn->Execute($csql);
	if($crs && !$crs -> EOF && intval($crs->fields["mcount"]) > 0){
		$sql = "update wdata_pic set title = 'club_title',detail='club_detail',";
		if($jp != ""){
			$sql = $sql."picture='$jp',";
		}
...
#		echo "update.sql:".$sql;
		if ($conn->Execute($sql)) {
				$location = "Location:".$url2."?hotid=".$hotid."&msg=modify completed success 1";
				header($location);	
				exit;		
		} else {
				$location = "Location:".$url2."?hotid=".$hotid."&msg=".$sql;
				header($location);	
				exit;		
		}	
		...
		if($jp != ""){
			$sql="insert into wdata_pic (hotid,picture,udate,title,detail,ordernum) values ('$hotid','$jp','$udate','$ptitle','$pdetail','$ordernum')";
			if ($conn->Execute($sql)) {
					$location = "Location:".$url2."?hotid=".$hotid."&msg=upload completed success 1";
					header($location);	
					exit;		
			} else {
					$location = "Location:".$url2."?hotid=".$hotid."&msg=upload completed failure 3";
					header($location);	
					exit;		
			}
		}else{
			$location = "Location:".$url2."?hotid=".$hotid."&msg=upload completed failure [no pic choose ]3";
			header($location);	
			exit;		
		}
	}

...
EDiaryEditor = {
	...
	initView: function (sParentID) {
		...
		var _toolBarInfo = [
			...
			{l: 454, t: 10, w: 24, h: 23, a: "插入动画", n: "flash"},
			{l: 480, t: 10, w: 23, h: 23, a: "添加附件", n: "attach"},
			{l: 506, t: 10, w: 24, h: 23, a: "插入链接", n: "link"},
			{l: 531, t: 10, w: 24, h: 23, a: "插入表格", n: "table"},
	...
	initControl: function (bWordMode, sContent) {
		...
		function viewDialog(el, Act) {
			...
			var win;
			if(window._isIE) {			
				window.showModalDialog("editor/"+Act+".asp", window, "dialogWidth: 400px; dialogHeight: 200px; help: no; scroll: no; status: no");
			}else {
				win = window.open("editor/"+Act+".asp", null, "Width=400,Height=200,left="+(window.screen.availWidth-400)/2+",top="+(window.screen.availHeight-200)/2+"");
		...
		function addFlash(event, Act) {
				var html = "<embed height='400' type='application/x-shockwave-flash' allowfullscreen='true' width='480' src='" + $("EDiaryEditorIMGInput").value + "' allowscriptaccess='always' wmode='opaque'></embed>";
				if (_this.iframe.contentWindow.document.selection.type.toLowerCase() != "none"){
					_this.iframe.contentWindow.document.selection.clear() ;
				}
				_this.iframe.contentWindow.focus();
		...
		$("EDiaryEditorflash").onclick = function (event) {
			viewDialog(event, "flash");
		}
	...
	runCMD: function (CMD, sValue) {
		...
		else if(CMD == "justifyformat" || CMD == "img" || CMD == "flash" || CMD == "link" || CMD == "table" || CMD == "attach" || CMD == "face") {

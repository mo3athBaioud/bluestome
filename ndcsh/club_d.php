<? 
	...
	$pmrs = $conn->Execute("select * from wdata_pic where hotid='".$hotid."' order by ordernum LIMIT ".$start." ,6");
...
<?	
	$dmrs = $conn->Execute("select * from wdata where mtype = 'b' and stype = 'b' order by ordernum");	
	while ($dmrs && !$dmrs->EOF) { 	?>				

...
public class AccessEndpoint extends AbstractEndpoint implements
    ...
    // 重连的响应超时时间
    private long retry2AccessTimeout = 30 * 1000;
    ...
    // 重连总超时时间
    private long totalRetryDataRecvTimeout = 120*1000;
                ...
                if(connectionState == CONN_STATE_ONLINE){
                    ...
                    if(isReconnect){
                        ...
                        if(isRetry2AccessTimeout()){
                            //重连是否超过允许重连的最大时间
                            if(!isTotalRetryTimeout()){
                                logger.debug("\t>>>>>> 重连未超过最大重连时间["+(totalDataRecvTimeout/1000)+"s] 或者重连次数小于 3 ,继续重连,当前为第"+(retry2accessCount ++)+"次重连");
                                sendReconnectSignal2Access();
                            }else{
                                logger.debug("\t>>>>>> 重连超过最大重连时间["+(totalDataRecvTimeout/1000)+"s] 执行UA注册\t连接状态:"+connectionState);
                                isRetry2AccessSend = false;
                                session.close(true);
                            }
                        }
    ...
    public void online(RegisterToAccessResp resp) {
        ...
        if (NetConstants.CONN_SUCCESS == resp.getResult()) {
            ...
            logger.info("\t>>>>>> register2access response failure!");
            connectionState = CONN_STATE_REGISTERING;
        }
    }
...
//            resetData();
//            sendRegisterWithoutProtocolToAccess();
//            connectionState = CONN_STATE_REGISTERING;
            session.close(true);
        }
    }

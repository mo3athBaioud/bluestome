...
public abstract class AbstractPollingIoConnector<T extends AbstractIoSession, H>
    ...
    protected final ConnectFuture connect0(
        ...
        try {
            ...
            if (connect(handle, remoteAddress)) {
                System.out.println("\t>>>>>> MINA连接时间:"+(System.currentTimeMillis()-start));
            ...
            System.out.println("\t>>>>>> try.catch.inner 耗时:"+(System.currentTimeMillis()-start));
            success = true;
        } catch (Exception e) {
            System.out.println("\t>>>>>> 连接异常:"+(System.currentTimeMillis()-start));
            e.printStackTrace();
            return DefaultConnectFuture.newFailedFuture(e);
        } finally {
            if (!success && handle != null) {
                System.out.println("\t>>>>>> connect0 耗时："+(System.currentTimeMillis()-start));
                try {
                    close(handle);
                    System.out.println("\t>>>>>> connect0.close(handle) 耗时："+(System.currentTimeMillis()-start));
                } catch (Exception e) {
                    ExceptionMonitor.getInstance().exceptionCaught(e);
                    System.out.println("\t>>>>>> connect0.Exception 耗时："+(System.currentTimeMillis()-start));
                }
            }
        }
        
        ConnectionRequest request = new ConnectionRequest(handle, sessionInitializer);
        System.out.println("\t>>>>>> connect0.ConnectionRequest 耗时："+(System.currentTimeMillis()-start));
        connectQueue.add(request);
        System.out.println("\t>>>>>> connect0.connectQueue.add(request) 耗时："+(System.currentTimeMillis()-start));
        startupWorker();
        System.out.println("\t>>>>>> connect0.startupWorker 耗时："+(System.currentTimeMillis()-start));
        wakeup();
        System.out.println("\t>>>>>> connect0.wakeup 耗时："+(System.currentTimeMillis()-start));
    ...
    public final class ConnectionRequest extends DefaultConnectFuture {
        ...
        public ConnectionRequest(H handle, IoSessionInitializer<? extends ConnectFuture> callback) {
            ...
            System.out.println("\t>>>>>> 程序中最终使用的超时时间:"+timeout);
            if (timeout <= 0L) {
                this.deadline = Long.MAX_VALUE;
            } else {
                this.deadline = System.currentTimeMillis() + timeout;
            }
            System.out.println("\t>>>>>> 程序中最终使用的结束点:"+deadline);
